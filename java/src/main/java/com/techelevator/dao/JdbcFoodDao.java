...
public class JdbcFoodDao implements FoodDao {
    ...
    @Override
    public Food getUserFoods(Food food) {

        String sql = "SELECT food_id, foods.food_name, calories, fat, protein, carbohydrate " +
                "FROM foods " +
                "JOIN user_foods ON foods.food_id = user_foods.food_id " +
                "WHERE user_foods.food_name ILIKE ?";

        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, food.getFood_name());
